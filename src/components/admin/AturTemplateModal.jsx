...
import { UploadCloud, X, File, Loader2, ArrowLeft } from "lucide-react";
import CertificateEditorCanvas from "./CertificateEditorCanvas";

export default function AturTemplateModal({ isOpen, onClose, onSave, savedTemplate }) {
  ...
  // Re-sync with savedTemplate whenever modal opens
  useEffect(() => {
    if (isOpen) {
      if (savedTemplate) {
        setFile(savedTemplate.file);
        setLayoutData(savedTemplate.layoutData);
        setStep(2);
      } else {
        setFile(null);
        setFileUrl(null);
  ...
  // Create object URL whenever file changes
  useEffect(() => {
    if (file) {
  ...
  return (
    <div className="fixed inset-0 z-[998] flex items-center justify-center bg-black/30 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className={`bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col transition-all duration-300 ${step === 2 ? 'w-[90vw] h-[90vh]' : 'w-full max-w-[500px]'}`}>
        {/* Header */}
        <div className="px-7 py-5 border-b border-gray-100 flex justify-between items-center bg-white">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-[17px] font-bold text-[#1E293B]">
              Atur Template Sertifikat {step === 2 && "- Pengaturan Tata Letak"}
            </h3>
          </div>
        ...
        {step === 1 ? (
          <div className="p-7 flex flex-col gap-4">
            <p className="text-[13px] text-gray-500">
              Unggah file template sertifikat. File harus berformat gambar (.png, .jpg) resolusi tinggi.
            </p>
              ...
              {file ? (
                ...
                <>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[12px] text-gray-400 mt-1">atau seret dan lepas file PNG/JPG di sini</p>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="flex-1 overflow-hidden p-4 bg-gray-50 min-h-0">
            {fileUrl && (
        ...
        {/* Footer */}
        <div className="px-7 py-5 border-t border-gray-100 flex gap-3 bg-white sticky bottom-0 mt-auto">
          <button
            type="button"
            onClick={onClose}
          ...
          {step === 1 ? (
            ...
            <>
              <button
                type="button"
                onClick={() => setStep(1)}
                className="flex-1 py-3 bg-white border border-gray-200 hover:bg-gray-50 text-gray-600 text-[13px] font-semibold rounded-xl transition-colors"
              >
                Ganti Desain
              </button>
              <button
              ...
              >
                {loading ? <Loader2 size={16} className="animate-spin" /> : "Simpan Template & Layout"}
              </button>
            </>
          )}
        </div>
